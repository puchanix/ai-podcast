...
export const personaConfig = {
  ...
  machiavelli: {
    id: "machiavelli",
    apiKey: "machiavelli",
    envVarName: "MACHIAVELLI_VOICE_ID",
    name: "Niccolò Machiavelli",
  },
  nietzsche: {
    id: "nietzsche",
    apiKey: "nietzsche",
    envVarName: "NIETZSCHE_VOICE_ID",
...
export const characterPrompts = {
  ...
  machiavelli:
    "You are Niccolò Machiavelli, the Renaissance political theorist. Speak pragmatically about power, politics, and human nature. Be strategic and realistic.",
  nietzsche:
    "You are Friedrich Nietzsche, the German philosopher. Speak boldly about morality, power, and the human condition. Be provocative and passionate.",
  gandhi:
...
export const personas = {
  ...
  machiavelli: {
    ...
    systemPrompt: "You are Niccolò Machiavelli, the Renaissance political theorist. Speak pragmatically about power.",
    prompt:
      "You are Niccolò Machiavelli, the Renaissance political philosopher. You speak with cold pragmatism about power, politics, and statecraft. You believe the ends often justify the means.",
    voiceId: process.env.MACHIAVELLI_VOICE_ID,
    getVoiceId: function () {
      if (voiceIdMap["machiavelli"]) {
        return voiceIdMap["machiavelli"]
      }
      if (this.voiceId) {
        return this.voiceId
      }
      if (process.env.MACHIAVELLI_VOICE_ID) {
        return process.env.MACHIAVELLI_VOICE_ID
      }
      return "echo"
    },
    podcast: "/podcast-machiavelli.mp3",
    questions: [
      "Is it better to be feared or loved?",
      "How should a leader maintain power?",
  ...
  nietzsche: {
    ...
    systemPrompt:
      "You are Friedrich Nietzsche, the German philosopher. Speak boldly about morality and human potential.",
    prompt:
      "You are Friedrich Nietzsche, the German philosopher who challenged traditional morality. You speak with passionate intensity about the will to power, the übermensch, and creating your own values.",
    voiceId: process.env.NIETZSCHE_VOICE_ID,
    getVoiceId: function () {
      if (voiceIdMap["nietzsche"]) {
        return voiceIdMap["nietzsche"]
      }
      if (this.voiceId) {
        return this.voiceId
      }
      if (process.env.NIETZSCHE_VOICE_ID) {
        return process.env.NIETZSCHE_VOICE_ID
      }
      return "echo"
    },
    podcast: "/podcast-nietzsche.mp3",
    questions: [
      "What does it mean to become who you are?",
      "Is traditional morality holding us back?",
